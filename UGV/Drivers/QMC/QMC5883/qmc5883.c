...
//<<<<<<<<<<<<<<<<-STATIC FUNCTIONS->>>>>>>>>>>>>>>>>>>
static HAL_StatusTypeDef UGV_getPositionAdc(Qmc * qmc);
 ...
 */
HAL_StatusTypeDef UGV_readData(Qmc * qmc)
{
	HAL_StatusTypeDef status;

	status = UGV_getPositionAdc(qmc);
	if(status != HAL_OK) return status;
 ...
 * @return none
 */
HAL_StatusTypeDef UGV_interruptEnable(Qmc * qmc)
{
	HAL_StatusTypeDef status;

	status = HAL_I2C_Mem_Read(i2c, QMC_SLAVE_ADDR, 1, QMC_CONTROL_REG2_ADDR,
							  &qmc->qmcReg.ControlReg2.reg, 1, 1);
	if(status != HAL_OK) return status;
 ...
 * @return none
 */
HAL_StatusTypeDef UGV_interruptDisable(Qmc * qmc)
{
	HAL_StatusTypeDef status;
